...
import {
  ...
  type OmpInput,
  type OmpLoadResult,
  type OmpProviderView,
  type OmpSettingsView
...
const draft = ref<OmpSettingsView>(emptyOmpSettings())
const saved = ref<OmpSettingsView>(emptyOmpSettings())
const expanded = ref('')
...
const apiOptions = computed(() => {
  const seen = new Set<string>(OMP_APIS)
  const options = OMP_APIS.map((value) => ({
    value,
  ...
  for (const provider of draft.value.providers) {
    ...
    seen.add(api)
    options.push({ value: api as OmpApi, label: api })
  }
  return options
})
...
const defaultModelChoices = computed(() => {
  ...
  for (const provider of draft.value.providers) {
    ...
    for (const model of provider.models) {
      ...
      if (!id) continue
      options.push({ id: `${key}/${id}`, ownedBy: model.displayName || key })
    }
  }
  return options
})
...
const unusedProviders = computed(() => {
  ...
  return savedProviders.value.filter((provider) => {
    ...
    return slug && isProviderKey(slug) && isOmpApi(provider.apiFormat) && !used.has(slug)
  })
})

const effortOptions = computed(() => {
  const { provider, model } = splitOmpDefault(draft.value.defaultModel)
  ...
  const levels = row?.thinkingLevels.length ? row.thinkingLevels : (fromHs?.thinkingLevels ?? [])
  return thinkingOptions(levels, draft.value.thinkingLevel)
})

function patch(partial: Partial<OmpSettingsView>): void {
  draft.value = { ...draft.value, ...partial }
}

function patchProvider(id: string, partial: Partial<OmpProviderView>): void {
  patch({
...
function capsFor(hs: ProviderConfig | null, modelId: string) {
  ...
  return {
    ...
    input: sanitizeOmpInput(fromProvider?.input)
  }
}

function modelChoicesFor(provider: OmpProviderView): FetchedModel[] {
  ...
  for (const model of provider.models) add(model.model, model.displayName || null)
  return options
}

function fromHyperSwitch(hs: ProviderConfig): OmpProviderView {
  const key = hs.slug.trim()
  return emptyOmpProvider({
    key,
    originalKey: '',
    api: isOmpApi(hs.apiFormat) ? hs.apiFormat : 'openai-completions',
    apiKey: hs.apiKey,
    baseUrl: hs.baseUrl,
    models: hs.models
...
function confirmImport(): void {
  ...
  const key = next.key
  if (!isOmpEditableKey(key, next.originalKey)) {
...
function providerTitle(provider: OmpProviderView): string {
...
function addModel(providerId: string): void {
  ...
  patchProvider(providerId, { models: [...provider.models, emptyOmpCatalogRow()] })
}
...
function updateModel(providerId: string, rowId: string, partial: Partial<OmpCatalogRow>): void {
  const provider = draft.value.providers.find((item) => item.id === providerId)
  if (!provider) return
  patchProvider(providerId, {
...
function toggleModelInput(providerId: string, rowId: string, type: OmpInput, enabled: boolean): void {
  const provider = draft.value.providers.find((item) => item.id === providerId)
  const row = provider?.models.find((item) => item.id === rowId)
  if (!row) return
  const current = sanitizeOmpInput(row.input)
  if (current.includes(type) === enabled) return
  updateModel(providerId, rowId, { input: toggleOmpInput(current, type) })
}
...
async function load(): Promise<void> {
  ...
  loading.value = true
  try {
    const file = (await window.hyper.loadAgentSettings('omp')) as OmpLoadResult
    ...
    toastStore.error(error instanceof Error ? error.message : t('editor.readFailed', { name: 'OMP' }))
  } finally {
    loading.value = false
  }
}
...
async function save(): Promise<void> {
  ...
  try {
    ...
    await window.hyper.saveAgentSettings('omp', { settings: snapshot, prompt })
    ...
    toastStore.success(t('editor.wrote', { name: 'OMP' }))
  } catch (error) {
...
<template>
  <div class="claude-editor">
    <p v-if="loading" class="hint">{{ t('editor.reading', { path: '~/.omp/agent/models.yml' }) }}</p>
    <template v-else>
      ...
      <div class="claude-block">
        ...
        <div v-if="draft.providers.length === 0" class="hint">{{ t('editor.emptyNamed', { name: 'OMP' }) }}</div>
            ...
            <div v-if="expanded === provider.id" class="oc-provider-body">
              <label class="field">
                ...
                <SelectMenu
                  ...
                  :options="apiOptions"
                  @update:model-value="patchProvider(provider.id, { api: $event })"
                />
              </label>
              <label class="field">
                ...
                >
                  ...
                  <div class="capability-row">
                    ...
                    <OptionChips
                      ...
                      :model-value="sanitizeOmpInput(row.input)"
