...
import {
  ...
  type DshInput,
  type DshLoadResult,
  type DshProviderView,
  type DshSettingsView
...
const draft = ref<DshSettingsView>(emptyDshSettings())
const saved = ref<DshSettingsView>(emptyDshSettings())
const expanded = ref('')
...
const apiOptions = DSH_APIS.map((value) => ({
  value,
  label: API_FORMAT_LABELS[value]
}))
...
const defaultModelChoices = computed(() => {
  ...
  for (const provider of draft.value.providers) {
    ...
    for (const model of provider.models) {
      ...
      if (!id) continue
      options.push({ id: `${key}/${id}`, ownedBy: model.displayName || provider.displayName || key })
    }
  }
  return options
})
...
const unusedProviders = computed(() => {
  ...
  return savedProviders.value.filter((provider) => {
    ...
    return (
      slug &&
      isProviderKey(slug) &&
      isDshApi(provider.apiFormat) &&
      !(DSH_RESERVED_ROUTES as readonly string[]).includes(slug) &&
      !used.has(slug)
    )
  })
})
...
const effortOptions = computed(() => {
  const { provider, model } = splitDshDefault(draft.value.defaultModel)
  const card = draft.value.providers.find((item) => item.key.trim() === provider)
  const row = card?.models.find((item) => item.model === model)
  const hs = matchSource(provider)
  const fromHs = hs?.models.find((item) => item.id === model)
  const levels = row?.thinkingLevels.length
    ? row.thinkingLevels
    : (fromHs?.thinkingLevels ?? [])
  return thinkingOptions(levels, draft.value.thinkingLevel)
})

function patch(partial: Partial<DshSettingsView>): void {
  draft.value = { ...draft.value, ...partial }
}

function patchProvider(id: string, partial: Partial<DshProviderView>): void {
  patch({
...
function capsFor(hs: ProviderConfig | null, modelId: string) {
  ...
  return {
    ...
    input: sanitizeDshInput(fromProvider?.input)
  }
}

function modelChoicesFor(provider: DshProviderView): FetchedModel[] {
  ...
  for (const model of provider.models) add(model.model, model.displayName || provider.displayName || null)
  return options
}

function fromHyperSwitch(hs: ProviderConfig): DshProviderView {
  const key = hs.slug.trim()
  return emptyDshProvider({
    key,
    originalKey: '',
    displayName: hs.name,
    api: isDshApi(hs.apiFormat) ? hs.apiFormat : 'openai-completions',
    apiKey: hs.apiKey,
    apiKeyEnv: dshApiKeyEnv(key),
    baseUrl: hs.baseUrl,
    models: hs.models
...
function confirmImport(): void {
  ...
  const key = next.key
  if (!isProviderKey(key) || (DSH_RESERVED_ROUTES as readonly string[]).includes(key)) {
  ...
  patch({
    ...
    defaultModel: draft.value.defaultModel || (first ? `${key}/${first.model}` : '')
  })
}

function providerTitle(provider: DshProviderView): string {
...
function removeProvider(id: string): void {
  ...
  if (current && defaultModel.startsWith(`${current.key}/`)) {
    const first = providers[0]?.models.find((row) => row.model.trim())
...
function addModel(providerId: string): void {
  ...
  patchProvider(providerId, { models: [...provider.models, emptyDshCatalogRow()] })
}
...
function updateModel(providerId: string, rowId: string, partial: Partial<DshCatalogRow>): void {
  const provider = draft.value.providers.find((item) => item.id === providerId)
  if (!provider) return
  patchProvider(providerId, {
...
function toggleModelInput(providerId: string, rowId: string, type: DshInput, enabled: boolean): void {
  const provider = draft.value.providers.find((item) => item.id === providerId)
  const row = provider?.models.find((item) => item.id === rowId)
  if (!row) return
  const current = sanitizeDshInput(row.input)
  if (current.includes(type) === enabled) return
  updateModel(providerId, rowId, { input: toggleDshInput(current, type) })
}
...
function toggleModelThinking(providerId: string, rowId: string, level: ThinkingLevel, enabled: boolean): void {
  ...
  updateModel(providerId, rowId, { thinkingLevels: toggleThinkingLevel(row.thinkingLevels, level) })
}
...
async function load(): Promise<void> {
  ...
  loading.value = true
  try {
    const file = (await window.hyper.loadAgentSettings('dsh')) as DshLoadResult
    ...
    toastStore.error(error instanceof Error ? error.message : t('editor.readFailed', { name: 'dsh' }))
  } finally {
    loading.value = false
  }
}
...
async function save(): Promise<void> {
  ...
  try {
    ...
    await window.hyper.saveAgentSettings('dsh', { settings: snapshot, prompt })
    ...
    toastStore.success(t('editor.wrote', { name: 'DeepSeek Harness' }))
  } catch (error) {
...
<template>
  <div class="claude-editor">
    <p v-if="loading" class="hint">{{ t('editor.reading', { path: '~/.dsh/settings.yaml' }) }}</p>
    <template v-else>
      ...
      <div class="claude-block">
        ...
        <p class="hint">{{ t('editor.keepExisting') }}</p>
        <div v-if="draft.providers.length === 0" class="hint">{{ t('editor.emptyNamed', { name: 'dsh' }) }}</div>
            ...
            <div v-if="expanded === provider.id" class="oc-provider-body">
              <label class="field">
                ...
                <SelectMenu
                  ...
                  :options="apiOptions"
                  @update:model-value="patchProvider(provider.id, { api: $event as DshApi })"
                />
              </label>
              <label class="field">
                ...
                >
                  ...
                  <div class="capability-row">
                    ...
                    <OptionChips
                      ...
                      :model-value="sanitizeDshInput(row.input)"
                      @toggle="(type, enabled) => toggleModelInput(provider.id, row.id, type, enabled)"
                    />
                  </div>
                  <div class="capability-row">
