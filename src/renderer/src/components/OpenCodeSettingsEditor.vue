...
import {
  ...
  emptyOpenCodeProvider,
  emptyOpenCodeSettings,
  npmFromFormat,
  OPENCODE_KEY_PATTERN,
  OPENCODE_NPM_PACKAGES,
  type OpenCodeLoadResult,
  type OpenCodeModelRow,
  type OpenCodeProviderView,
  type OpenCodeSettingsView
...
const draft = ref<OpenCodeSettingsView>(emptyOpenCodeSettings())
const saved = ref<OpenCodeSettingsView>(emptyOpenCodeSettings())
const expanded = ref('')
const revealKeys = ref<Record<string, boolean>>({})
const npmOptions = OPENCODE_NPM_PACKAGES.map((item) => ({ value: item.value, label: item.label }))
...
const defaultModelChoices = computed(() => {
  ...
  for (const provider of draft.value.providers) {
    ...
    for (const model of provider.models) {
      ...
      if (!id) continue
      options.push({ id: `${key}/${id}`, ownedBy: model.displayName || provider.name || key })
    }
  }
  return options
})
...
function patch(partial: Partial<OpenCodeSettingsView>): void {
  draft.value = { ...draft.value, ...partial }
}

function patchProvider(id: string, partial: Partial<OpenCodeProviderView>): void {
  patch({
...
function modelChoicesFor(provider: OpenCodeProviderView): FetchedModel[] {
...
function takenKeys(except = ''): string[] {
  return draft.value.providers.map((item) => item.key.trim()).filter((key) => key && key !== except)
}

function fromHyperSwitch(hs: ProviderConfig): OpenCodeProviderView {
  return emptyOpenCodeProvider({
    key: hs.slug.trim(),
    originalKey: '',
    name: hs.name,
    npm: npmFromFormat(hs.apiFormat),
    apiKey: hs.apiKey,
    baseUrl: hs.baseUrl,
    models: hs.models
...
function selectPicking(slug: string): void {
  pickingHsId.value = slug
}
...
function confirmImport(): void {
  ...
  if (!isProviderKey(key) || takenKeys().includes(key)) {
    toastStore.error(key ? t('editor.slugBad', { slug: key }) : t('editor.invalidSlug'))
    return
  }
  ...
  pickingHsId.value = ''
  patch({
    providers: [...draft.value.providers, next],
    defaultModel: draft.value.defaultModel || (next.models[0] ? `${next.key}/${next.models[0].model}` : '')
  })
}

function providerTitle(provider: OpenCodeProviderView): string {
  return matchSource(provider.key)?.name || provider.key
}

function removeProvider(id: string): void {
  const current = draft.value.providers.find((item) => item.id === id)
  const providers = draft.value.providers.filter((item) => item.id !== id)
  let defaultModel = draft.value.defaultModel
  if (current && defaultModel.startsWith(`${current.key}/`)) {
    const first = providers[0]?.models[0]
    defaultModel = first && providers[0] ? `${providers[0].key}/${first.model}` : ''
  }
  if (expanded.value === id) expanded.value = providers[0]?.id ?? ''
  patch({ providers, defaultModel })
}
...
function addModel(providerId: string): void {
  ...
  patchProvider(providerId, { models: [...provider.models, emptyOpenCodeModelRow()] })
}
...
function updateModel(providerId: string, rowId: string, partial: Partial<OpenCodeModelRow>): void {
  const provider = draft.value.providers.find((item) => item.id === providerId)
  if (!provider) return
  patchProvider(providerId, {
...
function modelThinking(row: OpenCodeModelRow): ThinkingLevel[] {
...
async function load(): Promise<void> {
  ...
  loading.value = true
  try {
    const file = (await window.hyper.loadAgentSettings('opencode')) as OpenCodeLoadResult
    ...
    toastStore.error(error instanceof Error ? error.message : t('editor.readFailed', { name: 'OpenCode' }))
  } finally {
    loading.value = false
  }
}
...
async function save(): Promise<void> {
  ...
  const keys = draft.value.providers.map((item) => item.key.trim())
  const missing = draft.value.providers.find((item) => !item.key.trim())
  if (missing) {
    toastStore.error(t('editor.needSlug'))
    return
  }
  const invalid = draft.value.providers.find((item) => !OPENCODE_KEY_PATTERN.test(item.key.trim()))
  if (invalid) {
    toastStore.error(t('providers.slugIllegal', { slug: invalid.key }))
    return
  }
  const duplicate = keys.find((key, index) => keys.indexOf(key) !== index)
  if (duplicate) {
    toastStore.error(t('providers.slugDup', { slug: duplicate }))
    return
  }
  saving.value = true
  try {
    ...
    await window.hyper.saveAgentSettings('opencode', { settings: snapshot, prompt })
    ...
    toastStore.success(t('editor.wrote', { name: 'OpenCode' }))
  } catch (error) {
...
<template>
  <div class="claude-editor">
    <p v-if="loading" class="hint">{{ t('editor.reading', { path: '~/.config/opencode/opencode.json' }) }}</p>
    <template v-else>
      <div class="claude-block">
        <label class="field">
          ...
          <ModelIdField
            ...
            :placeholder="t('editor.defaultModelPh')"
            @update:model-value="patch({ defaultModel: $event })"
            @select="patch({ defaultModel: $event })"
          />
        </label>
        <p class="hint">{{ t('editor.ocDefaultHint') }}</p>
      </div>

      <div class="claude-block">
        <div class="header-section-head">
          <h3>OpenCode {{ t('nav.providers') }}</h3>
          <div class="models-actions">
            <button class="btn btn-sm" type="button" @click="openAddPicker">{{ t('editor.addProvider') }}</button>
          </div>
        </div>
        <p class="hint">{{ t('editor.ocKeep') }}</p>

        <div v-if="draft.providers.length === 0" class="hint">{{ t('editor.emptyNamed', { name: 'OpenCode' }) }}</div>
        ...
        <article v-for="provider in draft.providers" :key="provider.id" class="oc-provider">
          <div class="oc-provider-head">
            <button class="oc-provider-toggle" type="button" @click="expanded = expanded === provider.id ? '' : provider.id">
              <svg
              ...
              <span class="oc-provider-meta">{{ provider.baseUrl || t('providers.noBaseUrl') }} · {{ t('providers.modelCount', { count: provider.models.length }) }}</span>
            </button>
          ...
          <div v-if="expanded === provider.id" class="oc-provider-body">
            <label class="field">
              <span class="field-label">{{ t('editor.sdk') }}</span>
              <SelectMenu
                :model-value="provider.npm"
                :options="[...npmOptions]"
                @update:model-value="patchProvider(provider.id, { npm: $event })"
              />
            </label>
            <label class="field">
            ...
            <label class="field">
              ...
              <span class="key-row">
                ...
                />
                <button class="btn" type="button" @click="revealKeys = { ...revealKeys, [provider.id]: !revealKeys[provider.id] }">
            ...
            <div class="agent-catalog-list">
              <AgentCatalogCard
                ...
                :key="row.id"
                :open="!row.model"
                @remove="patchProvider(provider.id, { models: provider.models.filter((item) => item.id !== row.id) })"
              >
                <template #head>
                  <input
          ...
          <section class="picker-panel" @click.stop>
            ...
            <div class="picker-body">
              <button
                ...
                type="button"
                @click="selectPicking(provider.slug)"
              >
                ...
                <div class="picker-row-copy">
                  ...
                  <div class="picker-row-id">
                    {{ provider.slug }} · {{ API_FORMAT_LABELS[provider.apiFormat] }} · {{ provider.baseUrl || t('providers.noBaseUrl') }}
                  </div>
                </div>
              </button>
            </div>
